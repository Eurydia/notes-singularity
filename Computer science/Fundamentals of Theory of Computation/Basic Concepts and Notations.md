...
Recall that; given a set $X$, we say that the power set of $X$ is a set containing all subset of $X$, denoted $\mathcal{P}(X):=\{A|A\subseteq X\}$.

**Definition**: Given an alphabet $V$, we define a family of languages over $V$ to be a set containing languages over $V$. Intuitively, we can build a hierarchy starting from a letter as;
- $V$ is an alphabet, elements of $V$ are letters,
- $V^{*}$ is a set containing all possible letter sequence in $V$, elements of $V^{*}$ are words,
- $\mathcal{P}(V^{*})$ is a set containing all subset of $V^{*}$, elements of $\mathcal{P}(V^{*})$ are languages, and
- $\mathcal{P}(\mathcal{P}(V^{*}))$ is a set of set of languages, elements of $\mathcal{P}(\mathcal{P}(V^{*}))$ are families of languages.

## Grammars

**Definition**: Given a quadruple $G=\langle N,\Sigma,P,S\rangle$ where $N,\Sigma$ are disjointed alphabets, $S$ is a letter in $N$, and $P$ is a set of ordered pairs $x,y$ where $x,y\in(N\cup\Sigma)^{*}$ where $x$ contains at least one letter from $N$, we say that $G$ is a *generative grammar* with $N$ as the set  of non-terminal symbols, $\Sigma$ as the set of terminal symbols, $S$ as the starting symbol, and $P$ as the set of production rules.  

### Derivation

**Definition**: Given a generative grammar $G=\langle N,\Sigma,P,S\rangle$ and two words $u,v\in(N\cup\Sigma)^{*}$, we say that we can derive $v$ from $u$ in one step in $G$ if $u=u_{1}au_{2}$ and $v=u_{1}bu_{2}$ where $u_{1},u_{2}\in(N\cup\Sigma)^{*}$ and $(a,b)\in P$.

We use $u\Rightarrow_{G}v$ to denote that we can derive $v$ from $u$ in one step in $G$.

**Definition**: Given a generative grammar $G=\langle N,\Sigma,P,S\rangle$ and two words $u,v\in(N\cup\Sigma)^{*}$, we say that we can derive $v$ from $u$ in multiple steps in $G$ if
- $u=v$, or
- there exists an integer $n\ge1$ and words $w_{0},\ldots,w_{n}$, where $w_{i}\in (N\cup\Sigma)^{*}$ for $i=0,\ldots,n$, such that $w_{i-1}\Rightarrow_{G}w_{i}$ for $i=1,\ldots,n$ where $w_{0}=u$ and $w_{n}=v$. 

We use $u\Rightarrow_{G}^{*}v$ to denote that we can derive $v$ from $u$ in multiple steps in $G$.

**Definition**: We say that the words we can derive from the starting symbol are the sentential form of $G$.

### Generated Languages

**Definition**: Given a generative grammar $G=\langle N,\Sigma,P,S\rangle$, we say that $L(G):=\{w|S\Rightarrow_{G}^{*}w\land w\in \Sigma^{*}\}$ is the language generated by a grammar $G$.